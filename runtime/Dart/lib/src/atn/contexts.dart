...
abstract class PredictionContext {
  ...
  static PredictionContext mergeSingletons(
      SingletonPredictionContext contextA,
    ...
    if (contextA.returnState == contextB.returnState) {
      var parent =
          merge(contextA.parent, contextB.parent, rootIsWildcard, mergeCache);
      if (parent == contextA.parent) return contextA;
      if (parent == contextB.parent) return contextB;
      var a =
          new SingletonPredictionContext.empty(parent, contextA.returnState);
  ...
  static PredictionContext mergeRoot(SingletonPredictionContext contextA,
      SingletonPredictionContext contextB, bool rootIsWildcard) {
    if (rootIsWildcard) {
  ...
  static PredictionContext mergeLists(
      ListPredictionContext contextA,
    ...
    while (
        i < contextA.returnStates.length && j < contextB.returnStates.length) {
      ...
      if (contextA.returnStates[i] == contextB.returnStates[j]) {
        ...
        bool both$ =
            payload == EMPTY_RETURN_STATE && aParent == null && bParent == null;
        bool axAx = (aParent != null && bParent != null) && aParent == bParent;
        if (both$ || axAx) {
          mergedParents[k] = aParent; // choose left
          mergedReturnStates[k] = payload;
        } else {
          var mergedParent =
              merge(aParent, bParent, rootIsWildcard, mergeCache);
  ...
  static PredictionContext getCachedContext(
    ...
    for (int i = 0; i < parents.length; i++) {
      var parent =
          getCachedContext(context.parentFor(i), contextCache, visited);
    ...
    if (parents.length == 0) {
      ...
      updated = new SingletonPredictionContext.empty(
          parents[0], context.returnStateFor(0));
    } else {
  ...
  static int _calculateHashCodes(
...
class ListPredictionContext extends PredictionContext {
  ...
  ListPredictionContext(List<PredictionContext> parents, List<int> returnStates)
      : this.parents = parents,
        ...
        super._internal(
            PredictionContext._calculateHashCodes(parents, returnStates)) {
...
class SingletonPredictionContext extends PredictionContext {
  ...
  factory SingletonPredictionContext.empty(
