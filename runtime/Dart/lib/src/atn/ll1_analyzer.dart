...
class Ll1Analyzer {
  ...
  IntervalSet look(AtnState state, RuleContext context, [AtnState stopState]) {
    ...
    var lookContext = context != null
        ? new PredictionContext.fromRuleContext(state.atn, context)
        : null;
    _look(state, stopState, lookContext, r, new HashSet<AtnConfig>(),
        new BitSet(), seeThruPreds, true);
    return r;
  }
      ...
      if (context != PredictionContext.EMPTY) {
        ...
        for (int i = 0; i < context.length; i++) {
          ...
          try {
            ...
            _look(returnState, stopState, context.parentFor(i), look, lookBusy,
    ...
    for (int i = 0; i < n; i++) {
      ...
      if (transition.runtimeType == RuleTransition) {
        ...
        try {
          ...
          _look(transition.target, stopState, newContext, look, lookBusy,
              calledRuleStack, seeThruPreds, addEof);
        } finally {
          calledRuleStack
              .clear((transition as RuleTransition).target.ruleIndex);
        }
      } else if (transition is AbstractPredicateTransition) {
        if (seeThruPreds) {
          _look(transition.target, stopState, context, look, lookBusy,
              calledRuleStack, seeThruPreds, addEof);
        } else {
