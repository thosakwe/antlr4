...
abstract class ErrorStrategy {
  ...
  ///
  /// [recognizer] is the parser instance.
  /// [exception] is the recognition exception to recover from.
  ///
  /// Throws [RecognitionException] if the error strategy could not recover
  /// from the recognition [exception]
  void recover(Parser recognizer, RecognitionException exception);

  /// This method provides the error handler with an opportunity to handle
  /// syntactic or semantic errors in the input source before they result in a
  /// [RecognitionException].
  ///
  /// The generated code currently contains calls to [sync] after entering the
  /// decision state of a closure block (`(...)*` or `(...)+`).
  ///
  ...
  /// [recognizer] is the parser instance.
  ///
  /// A [RecognitionException] occurs when an error is detected by the error
  ...
  /// symbols to the parse tree by calling [ParserRuleContext.addErrorNode]
  /// instead of [ParserRuleContext.addChild].
  ///
  /// [recognizer] is the parser instance.
  ///
  ...
  ///
  /// [recognizer] is the parser instance.
  /// [exception] is the recognition exception to report.
  void reportError(Parser recognizer, RecognitionException exception);
}
...
class DefaultErrorStrategy implements ErrorStrategy {
  ...
  bool inErrorRecoveryMode(Parser recognizer) => _errorRecoveryMode;

  IntervalSet getExpectedTokens(Parser recognizer) => recognizer.expectedTokens;
  ...
  void reportError(Parser recognizer, RecognitionException exception) {
    ...
    if (exception is NoViableAltException) {
      _reportNoViableAlternative(recognizer, exception);
    } else if (exception is InputMismatchException) {
      _reportInputMismatch(recognizer, exception);
    } else if (exception is FailedPredicateException) {
      _reportFailedPredicate(recognizer, exception);
    } else {
      print("unknown recognition error type: ${exception.runtimeType}");
      recognizer.notifyErrorListeners(
          exception.message, exception.offendingToken, exception);
    }
  }
  ...
  void recover(Parser recognizer, RecognitionException exception) {
    if (_lastErrorIndex == recognizer.inputStream.index &&
        _lastErrorStates != null &&
        _lastErrorStates.contains(recognizer.state)) {
    ...
    _lastErrorIndex = recognizer.inputStream.index;
  ...
  /// The default implementation of [ErrorStrategy.sync] makes sure that
  /// the current lookahead symbol is consistent with what were expecting
  ...
  Future sync(Parser recognizer) async {
    ...
    int la = await tokens.lookAhead(1);
    ...
    switch (state.stateType) {
      ...
      case AtnState.STAR_LOOP_ENTRY:
        ...
        throw new InputMismatchException(
            recognizer, await recognizer.currentToken);
  ...
  Token recoverInline(Parser recognizer) async {
    ...
    if (await _singleTokenInsertion(recognizer))
      return _getMissingSymbol(recognizer);
    // even that didn't work; must throw the exception
    throw new InputMismatchException(recognizer, await recognizer.currentToken);
  }
  ...
  Future _reportNoViableAlternative(
      Parser recognizer, NoViableAltException e) async {
    TokenStream tokens = recognizer.inputStream;
    String input;
    if (tokens is TokenStream) {
      if (e.startToken.type == Token.EOF)
        ...
        input = await tokens.getTextBetween(e.startToken, e.offendingToken);
    } else {
      input = "<unknown input>";
    }
  ...
  Future _reportUnwantedToken(Parser recognizer) async {
    if (inErrorRecoveryMode(recognizer)) return;
    beginErrorCondition(recognizer);
    Token token = await recognizer.currentToken;
    String tokenName = await _getTokenErrorDisplay(token);
  ...
  // error recovery mode. Otherwise, it calls beginErrorCondition to
  // enter error recovery mode, followed by calling Parser.notifyErrorListeners.
  Future _reportMissingToken(Parser recognizer) async {
    if (inErrorRecoveryMode(recognizer)) return;
    beginErrorCondition(recognizer);
    Token t = await recognizer.currentToken;
    IntervalSet expecting = getExpectedTokens(recognizer);
  ...
  Token _singleTokenDeletion(Parser recognizer) async {
    ...
    if (expecting.contains(nextTokenType)) {
      ...
      Token matchedSymbol = await recognizer.currentToken;
  ...
  Token _getMissingSymbol(Parser recognizer) async {
    ...
    Token lookback = await recognizer.inputStream.lookToken(-1);
    if (current.type == Token.EOF && lookback != null) {
      current = lookback;
    }
    return await recognizer.tokenFactory(
  ...
  Future _consumeUntil(Parser recognizer, IntervalSet set) async {
    int ttype = await recognizer.inputStream.lookAhead(1);
    while (ttype != Token.EOF && !set.contains(ttype)) {
      await recognizer.consume();
      ttype = await recognizer.inputStream.lookAhead(1);
    }
  }
}
